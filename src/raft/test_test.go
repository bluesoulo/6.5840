...
func TestReElection3A(t *testing.T) {
	...
	cfg.checkOneLeader()
	Log(dInfo, "S%d: test %d has completed!", 4, 1)
	...
	Log(dInfo, "S%d: test %d has completed!", 4, 2)
	...
	time.Sleep(2 * RaftElectionTimeout)
	Log(dInfo, "S%d: test %d has completed!", 4, 3)

	// check that the one connected server
	// does not think it is the leader.
	cfg.checkNoLeader()
	Log(dInfo, "S%d: test %d has completed!", 4, 4)
	...
	cfg.connect(leader2)
	cfg.checkOneLeader()
	Log(dClient, "S%d: test %d has completed!", 4, 6)
